...
let pageSize = 50;
let orderBy = 1;
let currentNameOrder = 'asc';


function toggle(orderOption) {
    console.log("changing from " + orderOption);
    if (orderOption === 'asc') return 'desc';
    else {
        return 'asc'
    }
}
...
router.get('/country', async (req, res) => {
    ...
    try {
        ...
        console.log('currentName Order = ' + currentNameOrder);
        currentNameOrder = toggle(currentNameOrder);
        console.log('currentName Order after = ' + currentNameOrder);
        let pageSize = parseInt(req.query.pageSize);
        let page = parseInt(req.query.page);
        // we retrieve all the countries
        const result = await countryRepo.getCountries(orderBy, currentNameOrder, pageSize, page);
